...
| 番号 | 内容 | 説明 |
| --- | --- | --- |
| 1 | アイテム名（商品名）が表示されます。 | 「[購入処理を予約する](https://developers.line.biz/ja/reference/line-mini-app/#reserve-purchase)」際に、[リクエストボディ](https://developers.line.biz/ja/reference/line-mini-app/#reserve-purchase-request-body)の`shopProductName`で指定します。<br />※ユーザーが購入したアイテムを特定できるよう適切な値を設定してください。 |
| 2 | アプリ内課金を利用する当社サービス名（LINEミニアプリ）と、サービス事業主のサービス名が表示されます。<br /><br />**表示パターン：**<ul><li>端末の言語設定が日本語：`LINEミニアプリ・{サービス事業主のサービス名}`</li><li>その他の言語：`LINE MINI App・{サービス事業主のサービス名}`</li></ul>※現時点ではサービス事業主のサービス名の多言語対応はしていません。 | サービス事業主のサービス名は、LINEミニアプリチャネルの［**チャネル基本設定**］タブ > 「**チャネル名**」で設定します。<br />※認証済ミニアプリのチャネル名を変更する場合は、[再審査](https://developers.line.biz/ja/docs/line-mini-app/service/update-service/)が必要です。 |
| 3 | 決済に使用したアプリストア（App StoreまたはGoogle Play）と、価格に応じたプロダクトIDが表示されます。 | プロダクトIDは当社があらかじめアプリストアに登録したもので、開発者が編集することはできません。プロダクトIDと価格の組み合わせは、[アプリ内課金で購入可能なアイテムのプロダクトID一覧](https://developers.line.biz/ja/docs/line-mini-app/in-app-purchase/iap-product-id/)で確認できます。 |
